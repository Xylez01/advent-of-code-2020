...
Future<List<String>> readInput() async =>
    (await new File('./day-13/input.txt').readAsString()).split('\r\n');
...
int findEarliestBus(int arrivalTime, List<int> busTimes) {
  final busses = busTimes
      .map((time) => Bus(time, (arrivalTime / time).ceil() * time))
      .toList()
...
bool fitsSchedule(int timeStamp, int offset, int id) =>
...
void main() async {
  ...
  final busTimes = busSchedule
      .where((character) => character != 'x')
